...
for(let elem of contents){
  ...
  style.innerText += "@keyframes " + elem.id + "-retracting { from { max-height: " + elem.clientHeight + "px; padding-bottom: 15px;} to { max-height: 0px; padding-bottom: 0px; } }\n";
  details.open = false;
  oldClasses.forEach((item) => {elem.classList.add(item)});
  ...
  details.children[0].onclick = event => {
    if(details.open){
      ...
      elem.style = "overflow: hidden; animation-name: " + elem.id + "-retracting; animation-duration: .3s; max-height: 0px; padding-bottom: 0px";
      chevron.style = "animation-name: chevron-rotate-back; transform: rotate(0deg);";
